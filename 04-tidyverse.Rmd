...
Every R script you write will require you to import a data file and create/save a new data file. 
...
You should always have in mind the final resulting dataframe you want to create. In this case, we want to calculate the mean and standard deviation reaction times (RT) for congruent and incongruent trials and for each subject individually. What we will end up with is a dataframe where each subject has two rows of data, one for congruent and one for incongruent conditions. Then columns specifying the Subject number, Condition, mean RT, and mean SD. That is four columns. The number of rows should be two times the number of subjects. Let's first evaluate how many subjects we have - so we can know exactly how many rows to expect. 
...
Cool, so in our final dataframe at the end of the R script we should expect to have **`r 2*length(unique(import$Subject))`** rows and **4** columns
...
Finally we arrive at `summarise()`. We've done our filtering, selecting, mutating, now let's aggergate RTs accross `Condition` to calculate mean and standard deviations. We will use a combo of `group_by()` and `summarise()`. `summarise()` is almost always used in conjunction with `group_by()`. 
...
```
To `summarise()` you need to create new column names that will contain the aggregate values. `RT.mean` and `RT.sd` seem to make sense to me.

How many rows and columns does `data` have? Recall we were expecting 820 rows with 4 columns. Is that right? Does each subject have two rows as expected?

Also notice how irrelevant columns got removed: `Trial`, and `ACC`.
...
**Cool! Before moving on to writing more R scripts, we need to talk about organization and workflow**