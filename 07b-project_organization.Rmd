...
*You can copy R Scripts from one data analysis project to another*. For instance, if you or someone else already created an R Script to score a particular task that you are also using, for another data analysis project, then of course there is no problem in copying the R Script. 
...
Unfortunately, you have to actually collect data before you can start analyzing anything. You start out with a single repository: **Data Collection**. It may be organized something like this
...
In addition to the **Tasks** and **Documents** folders, you will have a **Data Files** and **R Scripts** folders. The Data Files folder is where *ALL* the raw data files will be stored. 
...
After performing the 4 data preparation steps, the final data collection repository will end up looking something like:

```{r echo=FALSE, eval = TRUE, out.width='35%', fig.align='center'}
knitr::include_graphics(rep("images/repository_central.png"))
```

The tidy raw data files are saved as **task_raw.csv**. These are the files you will copy to a *Data Analysis* repository.

The *Data Collection* repository that you started out with will serve as the *Central Repository* that you use to create separate *Data Analysis* repositories. **You only need to copy the *tidy raw data* files from this central repository to multiple data analysis repositories**.
...
The **2_merge.R** script merges all the **task_Scores.csv** files together into one **Final_Data.csv** file that is ready for statistical analysis, located in **Data Files**.
...
I will show you how to automatically create **Data Collection** and **Data Analysis** repositories using RStudio Projects.

The most important thing to remember is that the **Data Collection** repository will serve as the **Central Repository** and that you only need to copy and paste the *tidy* **task_raw.csv** data files to a **Data Analysis** repository.
